export default ['$rootScope', '$scope', '$location', 'ConfigService', 'lastPath', '$http', 'Wait',
...
$scope.diffErrorMessage = "Error at ".concat('url').concat(" : ").concat(response.statusText);
...
function success(response) {
$scope.compareData = {};
$scope.compareData = response.data.compare.results.find((res, index) => {
...
$scope.diffView = view;
if (view == 'APP_CFG') {
document.getElementById('diffAppButton').className = "btn btn-xs btn-primary";
} else {
document.getElementById('diffAppButton').className = "btn btn-xs Button-primary--hollow";
}
if (view == 'DOMAIN_CFG') {
document.getElementById('diffDomainButton').className = "btn btn-xs btn-primary";
} else {
document.getElementById('diffDomainButton').className = "btn btn-xs Button-primary--hollow";
}
if (view == 'SOACONFIG') {
document.getElementById('diffSoaConfigButton').className = "btn btn-xs btn-primary";
} else {
document.getElementById('diffSoaConfigButton').className = "btn btn-xs Button-primary--hollow";
}
if (view == 'FMW_PATCH') {
document.getElementById('diffFmwButton').className = "btn btn-xs btn-primary";
} else {
document.getElementById('diffFmwButton').className = "btn btn-xs Button-primary--hollow";
}
if (view == 'DATABASE_CFG') {
document.getElementById('diffDatabaseButton').className = "btn btn-xs btn-primary";
} else {
document.getElementById('diffDatabaseButton').className = "btn btn-xs Button-primary--hollow";
}
if (view == 'OS_CFG') {
document.getElementById('diffOSButton').className = "btn btn-xs btn-primary";
} else {
document.getElementById('diffOSButton').className = "btn btn-xs Button-primary--hollow";
}
}
...
return propname.concat("(").concat(itemstatus).concat(")");
}

$scope.formatStatus = function(itemstatus) {
if (itemstatus == 'REMOVED') {
return 'Missing in Target Environment ('.concat($scope.env2).concat(')');
}
if (itemstatus == 'ADDED') {
return 'Missing in Source Environment ('.concat($scope.env1).concat(')');
}
}
...
Wait("start");
if (!$scope.isDisabledEnvironment($scope.env2)) {
var inventoriesUrl = '/api/v2/inventories/'.concat("?search=").concat($scope.env2);
var jobTemplatesUrl = '/api/v2/job_templates/?search=Update+configs';
var jobName = 'Update configs';
$http({method: 'GET', url: inventoriesUrl}).
then(
function success(response) {
var inventoryID;
var jobID;
...
$http({method: 'GET', url: jobTemplatesUrl}).
then(
function success(responseJobs) {
responseJobs.data.results.forEach(jobElement => {
if (jobElement.name == jobName) {
jobID = jobElement.id;
var sendURL = '/api/v2/job_templates/'.concat(jobID).concat('/launch/');
$http({method: 'POST', url: sendURL, data: msgToSend}).
then(function success(responseSend) {
$scope.diffErrorMessage = null;
$scope.diffSuccessMessage = "SUCCESS at ".concat(sendURL).concat(" : ").concat(responseSend.statusText);
$("html, body").animate({ scrollTop: 0 }, "slow");
},
function error(response) {
$scope.diffErrorMessage = "Error at ".concat(sendURL).concat(" : ").concat(responseSend.statusText);
$("html, body").animate({ scrollTop: 0 }, "slow");
})
}
})
...
$scope.diffErrorMessage = "Error at ".concat(jobTemplatesUrl).concat(" : ").concat(responseJobs.statusText);
$("html, body").animate({ scrollTop: 0 }, "slow");
});
...
if (!inventoryIDFound) {
$scope.diffErrorMessage = "Error at ".concat(inventoriesUrl).concat(" : ").concat("inventory ").concat($scope.env2).concat(" not found");
$("html, body").animate({ scrollTop: 0 }, "slow");
}
...
function error(response) {
$scope.diffErrorMessage = "Error at ".concat(inventoriesUrl).concat(" : ").concat(response.statusText);
$("html, body").animate({ scrollTop: 0 }, "slow");
Wait("stop");
}
);
} else {
$scope.diffErrorMessage = "Error : ".concat($scope.env2).concat(" environment is read-only!");
$('#diffApplyButtonAPP').css('cursor','not-allowed');
$('#diffApplyButtonDOMAIN').css('cursor','not-allowed');
$('#diffApplyButtonSOA').css('cursor','not-allowed');
$("html, body").animate({ scrollTop: 0 }, "slow");
...
var pdfTitle = 'ОТЧЕТ ПО ИТОГАМ СРАВНЕНИЯ КОНТУРОВ '.concat(env1).concat(' И ').concat(env2);
...
if (element.left_revision != null && element.right_revision != null &&
element.left_revision != element.right_revision) {
pdfTableAPPDifferenced.push([element.config, element.left_revision, element.right_revision]);
}
if (element.left_revision != null && element.right_revision == null) {
pdfTableAPPAbsent.push([element.config, element.left_revision, '-']);
}
if (element.left_revision == null && element.right_revision != null) {
pdfTableAPPAbsent.push([element.config, '-', element.right_revision]);                        
}
}
});
$scope.compareData.APP_CFG.data.forEach(element => {
...
pdfTableParametersDifferenced.push([{text:element.config, colSpan:3, alignment: 'center'},{},{}]);
}
pdfTableParametersDifferenced.push([element2.pathText.concat(' ').concat($scope.formatProperty(element3.name,element2.status.toUpperCase())), $scope.formatSourceValue(element3.left_value), $scope.formatSourceValue(element3.right_value)]);
...
if (element2.properties != null ) {
element2.properties.forEach(element3 => {
pdfTableSOADifferenced.push([element.config.concat('> ').concat(element2.pathText.concat(' ').concat($scope.formatProperty(element3.name,element2.status.toUpperCase()))), $scope.formatSourceValue(element3.left_value), $scope.formatSourceValue(element3.right_value)]);
})
}
...
if (element2.properties != null ) {
element2.properties.forEach(element3 => {
pdfTableDOMAINDifferenced.push([element.config.concat('> ').concat(element2.pathText.concat(' ').concat($scope.formatProperty(element3.name,element2.status.toUpperCase()))), $scope.formatSourceValue(element3.left_value), $scope.formatSourceValue(element3.right_value)]);
})
}
...
pdfTableFMWDifferenced.push([element.file,element.left_content_decoded,element.right_content_decoded]);
console.log(element.diff_lines);
} else {
pdfTableFMWDifferenced.push([element.file,'Различий нет','Различий нет']);
console.log(element.diff_lines);
}
});
$scope.compareData.DATABASE_CFG.data.forEach(element => {
if (element.left_content != null && element.right_content != null) {
pdfTableDATABASEDifferenced.push([element.file,element.left_content_decoded,element.right_content_decoded]);
console.log(element.diff_lines);
} else {
pdfTableDATABASEDifferenced.push([element.file,'Различий нет','Различий нет']);
console.log(element.diff_lines);
}
});
$scope.compareData.OS_CFG.left.forEach(element => {
element.diff.data.forEach(element2 => {
if (element2.left_content != null && element2.right_content != null) {
pdfTableOSLeftDifferenced.push([element.host, element2.file,element2.left_content_decoded,element2.right_content_decoded]);
} else {
pdfTableOSLeftDifferenced.push([element.host, element2.file,'Различий нет','Различий нет']);
}
});
});
$scope.compareData.OS_CFG.right.forEach(element => {
element.diff.data.forEach(element2 => {
if (element2.left_content != null && element2.right_content != null) {
pdfTableOSRightDifferenced.push([element.host, element2.file,element2.left_content_decoded,element2.right_content_decoded]);
} else {
pdfTableOSRightDifferenced.push([element.host, element2.file,'Различий нет','Различий нет']);
}
});
});
